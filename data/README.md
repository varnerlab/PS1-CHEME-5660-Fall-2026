...
`advanced-terms.csv` specifies the $100,000 year-7 liability, $10,000-par purchase lots, the common 4.25% annual note coupon, semiannual coupon frequency, horizon, and seven-year STRIP price per dollar of maturity value. Every strategy starts with `liability * seven_year_zero_price` USD. The lot size is an assignment constraint, not a universal Treasury purchase minimum.
...
For a zero with quoted price `V_B` per $100 par, L2a's price-implied growth rate is `g_B = log(100/V_B)/T`, where `T` is remaining years. L1b's benchmark is `g_y = 2*log(1 + y/2)` under `n = 2`; it equals `g_B` only when the purchase price equals the model price at the chosen valuation yield `y`. The finite-horizon `g_B` generally differs from the instantaneous CIR rate.
